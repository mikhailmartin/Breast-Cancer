...
def mini_etl(
    ...
    benign_tumor_data.drop(columns=['Дата рождения', 'Дата тестирования', '?1', '?2'], inplace=True)
    benign_tumor_data[src.constants.TARGET] = src.constants.LABELS[1]

    malignant_tumor_data = pd.read_excel(
        malignant_tumor_data_path, index_col=0, nrows=src.constants.MALIGNANT_TUMOR_N_ROWS)
    ...
    data = pd.concat([norm_data, benign_tumor_data, malignant_tumor_data], ignore_index=True)
    data.rename(columns={
    ...
    data[src.constants.QUESTION_6].replace({'нет ': src.constants.NO}, inplace=True)
    ...
    data[src.constants.QUESTION_16].replace({'-': np.NaN}, inplace=True)

    data[src.constants.QUESTION_17].replace({'-': np.NaN}, inplace=True)

    data[src.constants.QUESTION_18].replace({'-': np.NaN}, inplace=True)
    ...
    data.to_csv(etled_data_path, index=False)
